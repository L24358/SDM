...
gmavals = [[0.3,0.6,0.5,1,1], [0.3,0.6,0.6,1,1], [0.3,0.6,0.7,1,1],\
           [0.3,0.6,0.8,1,1], [0.5,0.6,0.7,1,1]] *6
...
def sim_seed(k): #tpe, gmas, c, rnum, snum, enum, tA=1, nsig=0.5, dt=0.0001
    ...
    def rhs_abv2(v, t):
        s1, s2, n1, n2 = v
        f1 = hpr.S_N(s1, gees*s1+gee*s2-m*(a*(s1+s2)+b)/2-n*(ag*((s1-s2)/2)**2+bg*((s1-s2)/2)+0)+Ie+hpr.pulse(hpr.Inp1(c), t-0.5)+n1)
        f2 = hpr.S_N(s2, gees*s2+gee*s1-m*(a*(s1+s2)+b)/2+n*(ag*((s1-s2)/2)**2+bg*((s1-s2)/2)+0)+Ie+hpr.pulse(hpr.Inp2(c), t-0.5)+n2)
